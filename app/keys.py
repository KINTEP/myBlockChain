...
#When we get a serialized SEC pubkey, we can write a parse method to figure out which y we need
    ...
    def parse(self, sec_bin):
        #This method returns the exact cordinates returned when the main class is called
        #Thus, S256Point(x = x, y = y) == S256Point(x = x, y = y).parse(__class__.sec())
        '''returns a Point object from a SEC binary (not hex)'''
        if sec_bin[0] == 4: #The uncompressed SEC format is pretty straightforward.
            x = int.from_bytes(sec_bin[1:33], 'big')
            y = int.from_bytes(sec_bin[33:65], 'big')
            return S256Point(x=x, y=y)
        is_even = sec_bin[0] == 2 #The evenness of the y coordinate is given in the first byt
        x = S256Field(int.from_bytes(sec_bin[1:], 'big'))
        # right side of the equation y^2 = x^3 + 7
        alpha = x**3 + S256Field(B)
        # solve for left side
        beta = alpha.sqrt() # We take the square root of the right side of the elliptic curve equation to get y
        if beta.num % 2 == 0: #We determine evenness and return the correct point
            even_beta = beta
...
class Signature:
    ...
    def parse(cls, signature_bin):
        #This function takes the self.der return values and converts it back to the original format in which
        #s and r were parsed into the class 
...
class PrivateKey:
    ...
    def deterministic_k(self, z):
        ...
        That is, it cannot get reused. In fact, a k that’s reused will result in you revealing your secret!
        """
        k = b'\x00' * 32
        v = b'\x01' * 32
        if z > N:
...
#WIF is a serialization of the private key that’s meant to be human-readabl
...
document = "This is my first document here"
...
print(s2)
pkkey = PrivateKey(secret = s2)

s1 = s2#int(pkkey.hex())

print(s1)
def sign1(s, z):
    key = PrivateKey(secret = s)
    key.sign(z)

    sig = key.sign(z)
    return sig

def verify(z, sig, secret):
...
v1 = verify(z=z, sig = sig, secret = s1)
print(v1)
