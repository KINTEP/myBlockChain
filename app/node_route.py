...
def add_to_chain():
	...
	if content:
			...
			#data_to = Transaction_Out(amount = , data = [data], pubkey = "")
			#trans = Transaction(input_ = data_from, output = data_to, secret = 12344556)
			#Mem = Mempool(content)
			#Mem.add()
			#unconfirmed_data = Mem.unconfirmed_data
		block = Block(transactions = content)
		global blockchain
...
def register_with_existing_node():
	...
	response = requests.post(node_address + "/register_new_peers", data = json.dumps(data),
				 headers = headers)
